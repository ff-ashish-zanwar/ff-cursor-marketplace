...
## Inputs
- JIRA key.
- Phase name (`jira-intake` | `completeness` | `routing` | `rca` | `plan` | `gate-1` | `base-branch-picked` | `code` | `review` | `gate-2` | `halted`).
...
### 1. Understand
Read the existing file if present. Reject a call if the phase ordering is wrong (e.g., `code` arriving before `base-branch-picked`, or `gate-2` arriving before `review`).
...
### 5. Implement
- Frontmatter: update `last-phase`. For `base-branch-picked`, append `{repo, base, feature}` entries to `branches` with empty `commit_sha`. For `code`, back-fill `commit_sha` on each matching `branches` entry.
