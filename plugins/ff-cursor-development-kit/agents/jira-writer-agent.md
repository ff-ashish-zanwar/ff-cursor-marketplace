...
- **Assignee — default to the CREATOR:** on every issue this run creates (parent + sub-tasks), set assignee to the **authenticated JIRA user running the command** (resolve via `myself` — the Atlassian MCP identity or the `JIRA_EMAIL` credential). A Gate-P override (user named someone else) wins. NEVER assign any other account, never a hardcoded one, and never rely on the project's default assignee (that silently lands tickets on the project lead).
