...
jira-agent
  ...
  → stop (changes stay uncommitted; developer stages, commits, pushes, then raises the MR)
```

Every agent above MUST banner per [`agent-attribution`](../rules/agent-attribution.md) before its output — H3 heading `### ▸ [<N>/12] <agent-name>` on line 1, italic action on line 2. The Review-Readiness Gate is `### ▸ [8/12] Review-Readiness Gate`. The 14 reviewers share one grouped banner (`### ▸ [9/12] review-agents (×14, parallel)` / `*analysing the uncommitted working-tree diff*`); per-reviewer banners are suppressed.
...
## Outputs
- Updated `<product>-ai-brain/task-history/<JIRA-KEY>.md`.
...
- **Post-Gate-2 finalize**: when the developer approves Gate 2, the orchestrator MUST (1) call `task-history-writer` with phase `gate-2` to append the final section to `<product>-ai-brain/task-history/<KEY>.md`, (2) verify the file exists with `last-phase: gate-2` stamped, (3) emit the completion panel per `pipeline-checklist`. The run does not end until all three are done. The changes remain uncommitted — the panel tells the developer to stage, commit, and push. See `human-approval-gates` constraints.
...
2. **Summary table** — rows: `Repo`, `Feature branch`, `Base`, `Working tree` (always `uncommitted`), `File(s) changed`, `Review result`, `JIRA comment`, `Task history`. The `Task history` row MUST contain the verified file path (`<product>-ai-brain/task-history/<KEY>.md`).
...
If task-history verification fails in step (1) of the finalize sequence, the orchestrator MUST NOT emit "complete" — it emits the halt banner (`### ▸ [12/12] task-history finalize` / `*HALT after Gate 2: finalize failed at <product>-ai-brain/task-history/<KEY>.md*`) and stops. This is the only signal that the file is missing.
