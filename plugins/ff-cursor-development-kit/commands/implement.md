...
4. **Publish reminder (Step 13)** — `<KEY>.md` was written with `published: false`. List every still-pending record (`published: false` under `<product>-ai-brain/task-history/`): *"📌 Push pending — run `/publish-history` to branch off `development` and raise an MR: `<file names>`."* The pipeline never pushes the record; the developer publishes when ready (`/publish-history` raises the MR; a separate agent merges).
